...
function smellToRange(
  ...
  if (end.isBeforeOrEqual(start)) {
    const safeEndCharacter = Math.min(
      start.character + 1,
      startLine.text.length,
    );
    return new vscode.Range(
      start,
      new vscode.Position(start.line, safeEndCharacter),
    );
  }
...
function buildDiagnosticMessage(smell: TestSmell): string {
...
export function mapSmellsToDiagnostics(
  ...
  return smells.map((smell) => {
    const message = buildDiagnosticMessage(smell);
    const diagnostic = new vscode.Diagnostic(
      smellToRange(smell, document),
      message,
      severity,
    );
    diagnostic.source = "SNUTS.js";
    diagnostic.code = smell.description ?? "test-smell";
    return diagnostic;
  });
}
